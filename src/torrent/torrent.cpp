...
void Torrent::load(const QByteArray &data, bool verbose)
{
    ...
    if (meta_version == 1)
        ...
        SHA2HashGen hg2;
        SHA2Hash hash2 = SHA2Hash(hg2.generate(metadata));
...
{
    BDictNode *info = dict->getDict("info");
    if (!info) {
        throw Error(i18n("Corrupted torrent."));
    }

    const BValueNode *m = info->getValue("meta version");
    meta_version = m ? m->data().toInt() : 1;
    if (meta_version > 2)
        throw Error(i18n("Unsupported torrent version: %1.", meta_version));

    chunk_size = info->getInt64("piece length");

    BDictNode *file_tree = info->getDict("file tree");
    BDictNode *piece_layers = dict->getDict("piece layers");
    ...
    BListNode *files = info->getList("files");
    if (files) {
        loadFilesV1(files);
    } else if (meta_version == 1) {
        total_size = info->getInt64("length");
    }
...
void Torrent::loadHashV1(BDictNode *dict)
{
    ...
    const auto hash_string = dict->getByteArrayView(QByteArrayLiteral("pieces"));
    for (int i = 0; i < hash_string.size(); i += 20) {
        const SHA1Hash hash(hash_string.sliced(i, 20));
