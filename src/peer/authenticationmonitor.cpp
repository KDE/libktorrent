    ...
    while (itr != auths.end()) {
        ...
        if (ab->isFinished()) {
            ...
            if (socket) {
                ...
                if (dev) {
                    const net::Poll::Mode m = socket->connecting() ? Poll::Mode::OUTPUT : Poll::Mode::INPUT;
                    dev->prepare(this, m);
                }
            }
            ++itr;
    ...
    while (itr != auths.end()) {
        ...
        if (ab->isFinished()) {
            ...
            if (socket) {
                ...
                const bool r = dev && dev->ready(this, Poll::Mode::INPUT);
                const bool w = dev && dev->ready(this, Poll::Mode::OUTPUT);
                if (r) {
                    ab->onReadyRead();
                }
                if (w) {
