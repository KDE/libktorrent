...
void PeerManager::pex(QByteArrayView arr, int ip_version)
{
    if (!d->pex_on) {
        return;
    }
    ...
    if (ip_version == 4) {
        ...
        for (int i = 0; i + 6 <= arr.size(); i += 6) {
            addPotentialPeer(net::Address::fromCompactIPv4(arr.sliced(i, 6)), false);
        }
    } else if (ip_version == 6) {
        Out(SYS_CON | LOG_NOTICE) << "PEX: found " << (arr.size() / 18) << " IPv6 peers" << endl;
        for (int i = 0; i < arr.size(); i += 18) {
            addPotentialPeer(net::Address::fromCompactIPv6(arr.sliced(i, 18)), false);
        }
    }
}
...
QList<Peer *> PeerManager::getPeers() const
{
    ...
    for (const auto &[peer_id, peer] : d->peer_map) {
...
void PeerManager::Private::update()
{
    ...
    for (PeerMap::iterator i = peer_map.begin(); i != peer_map.end();) {
        std::unique_ptr<Peer> &peer = i->second;
...
{
    for (const auto &[peer_id, peer] : std::as_const(peer_map)) {
